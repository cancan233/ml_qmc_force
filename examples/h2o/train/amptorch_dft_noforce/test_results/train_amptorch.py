...
images = tr("../../../data/dft/train_dft_pyscf_ase_force.traj")
print(len(images))
...
trainer.load_pretrained(
   "../checkpoints/2022-05-09-22-16-49-test"
)
...
# test_images = tr("../../../data/dft/test_dft_pyscf_ase_force.traj")
...
validate_images = tr("../../../data/dft/theta_with_forces.traj")
predictions = trainer.predict(validate_images)

true_energies = np.array([image.get_potential_energy() for image in validate_images])
pred_energies = np.array(predictions["energy"])
pred_forces = np.array(predictions["forces"])
...
print("TEST Energy MAE:", np.mean(np.abs(true_energies - pred_energies)) / 3)
print("TEST Energy RMSE:", np.sqrt(np.mean((true_energies - pred_energies) ** 2)) / 3)
amp_dmc_traj = tw("h2o_amptorch_dmc_theta.traj")
print(len(validate_images))
for i in range(len(validate_images)):
    atoms = validate_images[i]
    atoms.info["true_energy"] = true_energies[i]
    amp_dmc_traj.write(atoms, energy=pred_energies[i], forces=pred_forces[i])

validate_images = tr("../../../data/dft/r1_with_forces.traj")
predictions = trainer.predict(validate_images)
...
print("TEST Energy MAE:", np.mean(np.abs(true_energies - pred_energies)) / 3)
print("TEST Energy RMSE:", np.sqrt(np.mean((true_energies - pred_energies) ** 2)) / 3)
amp_dmc_traj = tw("h2o_amptorch_dmc_r1.traj")
print(len(validate_images))
for i in range(len(validate_images)):
    atoms = validate_images[i]
    atoms.info["true_energy"] = true_energies[i]
...
validate_images = tr("../../../data/dft/r2_with_forces.traj")
predictions = trainer.predict(validate_images)
...
print("TEST Energy MAE:", np.mean(np.abs(true_energies - pred_energies)) / 3)
print("TEST Energy RMSE:", np.sqrt(np.mean((true_energies - pred_energies) ** 2)) / 3)
amp_dmc_traj = tw("h2o_amptorch_dmc_r2.traj")
print(len(validate_images))
for i in range(len(validate_images)):
    atoms = validate_images[i]
    atoms.info["true_energy"] = true_energies[i]
