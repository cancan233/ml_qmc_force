...
images = tr(
    "/users/chuang25/data/chuang25/github_repo/ml_qmc_force/examples/ch3cl/dft_pyscf_ase/dft_pyscf_ase_force.traj"
)
...
amp_dft_traj = tw("amptorch_dft_force.traj")
print(len(images))
for i in range(len(images)):
    atoms = images[i]
    amp_dft_traj.write(atoms, energy=pred_energies[i], forces=pred_forces[i])


print("Energy MSE:", np.mean((true_energies - pred_energies) ** 2) / 5)
print("Energy MAE:", np.mean(np.abs(true_energies - pred_energies)) / 5)
print("Energy RMSE:", np.sqrt(np.mean((true_energies - pred_energies) ** 2)) / 5)
